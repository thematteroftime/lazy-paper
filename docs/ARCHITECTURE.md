...
4. **Verify**: `verify_section_draft(draft, chunks_by_id, ratio_threshold)` checks each claim's `cited_quote` with a four-tier match (exact substring → case-insensitive substring → normalized substring with LaTeX commands stripped + OCR digit-spacing folded → fuzzy longest-common-substring). If the cited chunks don't match, the verifier scans ALL retrieved chunks; on match the claim is accepted and `cited_chunk_ids` is patched (chunk-ID slop tolerance). Threshold env: `LAZY_PAPER_VERIFIER_THRESHOLD` (default 0.85). An **anchor advisory** then runs: when the claim names a specific author (`X et al.` / `X 等人`) or numeric value (`2.94 J/cm³`, `91.04%`, …), the verifier records `anchor_missing` in `critic_flags.yaml` when the `cited_quote` doesn't contain any of those anchors. This is **advisory only** as of v1.8.3 — claims are still accepted so the LLM's good-content-with-poorly-aligned-quote case (common on the meng2024 benchmark) doesn't lose comparator citations. An earlier enforcement variant was reverted.
