...
- **v1.4 plan** (planning, 2026-05-20): research-validated 2-stage roadmap in `docs/v1_4_content_roadmap.md`. (1) In-house RAG over paper (replaces keyword scoring with embedding retrieval scoped to current section; ~200 LOC, do **not** depend on PaperQA2). (2) Self-Refine critic loop — Python regex tier first (numeric / Fig.N / chem-formula checks against source), LLM tier only when regex flags problems. Multi-agent outliner deferred to v1.5; DSPy downgraded to Signature-only in v2.0. **LangGraph rejected** — our deterministic 9-stage DAG doesn't need its checkpointing / agent branching / human-in-loop primitives; ~30 LOC of `StageContext + @retry + while-loop` is the simpler equivalent. Cost budget revised to +100% per section (research showed earlier +30% draft was unrealistic). ~6 days implementation. Awaiting maintainer greenlight.
