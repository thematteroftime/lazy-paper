...
2. **Always run `uv run pytest -q` before and after any change**. If you break tests, fix them or revert. 228 should pass; live-LLM tests are gated behind `-m live`.
...
1. `uv run pytest -q` → 228 pass, 2 deselected.
