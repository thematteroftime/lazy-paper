...
```bash
uv run pytest -q          # 158 个测试
uv run pytest -m live     # 真 LLM 烟测（需要真实 key）
```

## 已知问题（v1.1.0）

PPT 渲染有两处边角 case 已诊断、待 v1.2 修：数学下标字体回退（罕见的 Unicode 下标在缺 `Noto Sans Math` 的机器上会回退到普通 ASCII），以及节内要点卡在 ≥6 条且单条换行时的视觉重叠。完整分析与修法见 [`docs/PPT_KNOWN_ISSUES.md`](docs/PPT_KNOWN_ISSUES.md)。
