...
> **状态：** 已发布 · **测试：** 250/250 通过 · **已在 13+ 篇论文上完成端到端验证**（v1.8.2 的 10 篇语料库 + v1.8.1 的 3 篇稳定性验证）· **最近发布：** v1.8.2 (2026-05-21)
>
> **v1.8.1 将 Strategy KL 确立为推荐的高质量默认方案。** 修复了
> v1.7 KL 出现 1 – 13 大方差的两个 compose 端 bug：(a) verifier
> 在子串匹配前对 LaTeX/OCR 形式做归一化，因此引用 comparator 的
> 优质 claim 不再因为 `$W _ { \mathrm { rec } }$` 之类的空白差异
> 被错误拒绝；(b) retry-when-empty 触发条件改为在 verify 之后
> 测量覆盖率，因此 verifier 刚刚 drop 掉 comparator claim 时
...
| 论文 | 章节数 | 图片数 | PPT 幻灯片数 | 大纲分组数 | 是否验证 |
|---|---|---|---|---|---|
| he2023 | 15 | 8 | 12 | 4 | ✓ |
...
5 篇论文都能干净地产出 DOCX/PDF/HTML（已验证到 v15）。输出路径：`runs/<paper_id>/s09_render/preview.pptx`。

**测试**：189 个单元 + 集成（其中 2 个标记 `-m live` 被默认跳过）。运行命令：`uv run pytest -q`。
