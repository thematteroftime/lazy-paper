...
>
> **v1.11.2** 是 tooling/erratum 发布 —— 相对 v1.11.1，流水线行为零变化。cycle 12 audit #1 报告 ali2025_flash ch13 捏造三个 baseline 数值；2 个独立 meta auditor 随后推翻该报告（OCR 把数值输出成了 LaTeX 间隔形式 `1 7 . 3` —— plain `grep` 漏抓）。候选 hotfix 加了 prompt rule + numeric verifier advisory，在 commit 前被 revert，因为 spec 确认它会在真章节引入真回归。最终上线的是：`scripts/audit_grep.py`（OCR-tolerant 替代品）+ TEST_FRAMEWORK 中"审计陷阱"一节，避免 subagent 审计重蹈方法论错误。
>
> **已知 v1.11.3 候选**（推迟到独立 diagnostic cycle）：meng2024 ch06 把 `W_rec=5.00 J/cm³ / η=90.09%` 错绑到 "180 kV/cm"（源论文是 340 kV/cm），并捏造 `E_b=214 kV/cm`（源论文无此值）；meng2024 ch07 在 v1.10 → v1.11.1 之间丢掉了 `5.00 J/cm³` 引用。一次单行修复尝试（把 retriever cap 12 改 24）已被 revert（ch06 没修好 + 章节字数塌缩 60%）。真正原因是 v1.10→v1.11.1 STRUCTURED 路径迁移留下的结构性问题，需要独立审计周期，而不是一行 patch。
>
...
>
> **v1.11.0** 是一次 first-principles 重构（commit `a4d90ab`），从 v1.10 主动**删掉**了 3 个过度设计的模块：cross-citation reject（~40 LOC）、figure-retry pass（~85 LOC）、headline-metric prompt rule。理由见 `docs_zh/ARCHITECTURE.md` §11。
>
> **v1.10.0** 上线 **Variant C — figure_ids 硬约束**：schema-level figure citation + figure-retry pass + env-gated whitelist。从 3-variant × 9-paper × 3-audit-cycle 测试（33 LLM run、9 specialist auditor）里选出。在多图论文上跑出真 100% figure embed（ali2025_flash 26/26、hif_1 20/20、hif_2 17/17），保住 meng2024 T1 = 9/9/9 零方差，在 ali2025_flash T4 上破基线（4→5）。同时上线 `normalize_ocr_latex` BS3+BS4（LaTeX escape 折叠 + Unicode NFKD）提升所有 variant 的 verifier 精度。完整验证见 `docs/archive/v1_10_variant_comparison.md`。
>
...
>
> **v1.9.0 上线 informed-retry，把 meng2024 T1 的方差降到 0。** 之前的 retry-when-empty 用的是笼统的"你漏了 required mentions"提示。v1.9 改为：每个缺失的 required mention 都生成一条逐实体诊断，附上具体锚点 token（作者姓氏 或 关联数值），LLM 拿到的是一份确定性检查清单。**meng2024 三次独立运行在 T1 上全部 9/17** —— 方差从 v1.8.1 的 stdev 2.6 降到 v1.9 的 **stdev 0**。完整分析见 `docs_zh/archive/v1_9_validation_results.md`。
>
...
如果你是冷启动接手本项目（无论是人类维护者还是 AI agent），请先读这份文档。它告诉你：项目里有什么、什么能用、哪些已经验证过、以及该在哪里改。
...
PDF → 9 个阶段 → 4 种输出格式。s01–s04 做 OCR、清洗、分章、图片提取（确定性，不调 LLM）。s05 解析大纲模板。s06–s08 是三个 LLM 驱动的阶段（论文上下文、单图分析、单章 compose）。s09 构造一个不可变的 `Document` 模型，分发给 4 个渲染器（docx/html/pdf/pptx），每个渲染器都是 `Renderer` 的无状态子类。PPTX 渲染器额外会调用 `PptxSummarizer` 做 4–5 组分组（`summarize_outline`）、单章 bullet（`summarize`）和收尾页（`summarize_paper`）；这些都用 input-hash 做了缓存，所以输入不变时重跑零 LLM 调用。每次 LLM 调用都会把 prompt + response 写到磁盘以便审计。每个阶段都会写 `done.yaml`，重跑时如果没加 `--force` 就会被跳过。完整的逐阶段说明见 `docs_zh/ARCHITECTURE.md`。
...
最近一次大规模验证：**v1.10 — 3-variant × 9-paper × 3-audit-cycle 测试（2026-05-23）**，完整报告 `docs/archive/v1_10_variant_comparison.md`；v1.11.1 又做了 cycle-11 sentence-level audit 修了 4 个 HIGH bug（见 banner）。
...
variant A/B 的 M2/M4 对比 + 零方差 stdev 表见 `docs/archive/v1_10_variant_comparison.md` §3。

**重要 — 环境变量解锁 M2 figure binding**：100% 那一列依赖 `LAZY_PAPER_FIGURE_BIND=1`。不开 variant C 仍受益（LLM 在 schema + base prompt 下会主动引图），但比率更低 — 见 `.env.example` 推荐 env 组合 + `docs/archive/v1_10_variant_comparison.md §7` 的 env-on vs env-off 对比。
...
| 新增一个流水线阶段 | 见 `docs_zh/ARCHITECTURE.md` 的 "Adding a new LLM stage" |
...
1. 先读本文件。
2. 读 `docs_zh/ARCHITECTURE.md` 了解逐阶段契约。
