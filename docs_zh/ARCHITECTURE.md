...
`context.yaml` 写入完成后，`kg_extract.build_paper_kg()` 使用 `instructor`（类型化的 Pydantic LLM 输出）从全部章节文本中抽取结构化知识图谱。默认 10 种类型封闭 schema：
...
设置 `LAZY_PAPER_KG_PROMPT=paper_kg_v3.md` 会新增第 11 种 `author`，并通过 `cited_by_paper` 关系链接到 comparator。Strategy KL 用这一版本把文献引用呈现为 "<Author> et al." 而非裸化学式。
...
s08 有两条共享相同输入/输出契约的路径。默认（"Strategy KL"）是所有生产 run 的推荐路径；legacy fallback 给没有 retriever 或 KG 的环境用。

#### 默认路径：结构化 compose + verifier（Strategy KL）

当 `LAZY_PAPER_STRUCTURED=1` 且 retriever 与 KG 均可用时，runner 使用 `stages/s08_section_compose/structured.py::compose_structured`。每个 section 流程：

1. **检索**：`retriever.retrieve(query, top_k=15)` 返回 RRF 融合（dense 余弦 + BM25）后的 chunk。
2. **必含提及**：`build_required_mentions` 选定 KG 实体（综述章节使用全部 comparator；非综述使用与 guidance token 重叠的实体），并把每个实体解析到一条覆盖它的 retrieved chunk 索引 + 通过 `cited_by_paper` 链接到 author。`select_top_required(cap=5)` 按长度 + 数字密度排序。
3. **Best-of-N compose**（环境变量 `LAZY_PAPER_BEST_OF_N`，默认 1，推荐 2）：在温度 0.2、0.4、0.6…… 下进行 N 次独立的 `instructor + SectionDraft` 调用。Pydantic 校验器会拒绝 `cited_chunk_ids` 超出 retrieved 集合的草稿。草稿通过轮询交错 + `_claim_dedup_key`（按共享 `(author, value)` 锚点合并 paraphrase）做并集合并。
4. **Verify**：`verify_section_draft(draft, chunks_by_id, ratio_threshold)` 用四档匹配校验每条 claim 的 `cited_quote`（精确子串 → 大小写不敏感子串 → 归一化子串：剥 LaTeX 命令 + 折叠 OCR 数字空白 → 模糊最长公共子串）。若所引 chunk 未匹配上，verifier 会扫描全部 retrieved chunk；匹配成功则接受 claim 并把 `cited_chunk_ids` 纠正（chunk-ID 容错）。阈值环境变量：`LAZY_PAPER_VERIFIER_THRESHOLD`（默认 0.85）。随后做**锚点 advisory**：当 claim 命名了某个 author（`X et al.` / `X 等人`）或具体数值（`2.94 J/cm³`、`91.04%` …），verifier 在 `cited_quote` 不含任何锚点时把 `anchor_missing` 写入 `critic_flags.yaml`。v1.8.3 起**仅作为建议**——claim 仍然接受，避免误伤"LLM 写对了内容但引用 quote 没对齐"这种 meng2024 benchmark 上常见的情况。之前的强制拒绝版本已经回退。
5. **Retry-when-empty**：对**经过 verify 的草稿**计算 `missing_required`。若 `post_cov ≤ LAZY_PAPER_RETRY_THRESHOLD`（默认 0.5），再发一次 `_single_compose`，使用强化系统 prompt；重新 verify；如覆盖度提升则替换。
6. **Retry-when-short**：即使覆盖率合格，若 verified 章节字符数低于 `LAZY_PAPER_MIN_SECTION_CHARS`（500）或 claim 数低于 `LAZY_PAPER_MIN_SECTION_CLAIMS`（4），再发一次 LLM 调用让其补写 distinct 事实；只有新结果更长时才替换。
7. **渲染**：`draft.render(mode="REMOVE")` 将 SectionDraft 平铺为正文；HTML 渲染器再把同样的 span 标记后处理成可点击的逐 claim 锚点（默认 HYPERLINK 模式）。

验证：`docs_zh/v1_8_validation_results.md` 与 `docs_zh/v1_8_2_corpus_validation.md` 给出 13 篇语料的基准分数。

#### Legacy fallback：prompt-stuffed compose

当 `LAZY_PAPER_STRUCTURED` 未设置，或 retriever / KG 任何一个未能构建时，runner 调用 `_legacy_compose`。这是 v1.4 路径：每章节单次 LLM 调用，把检索到的证据塞进 prompt，随后跑两层 critic（regex + LLM）。critic 把 flag 写入 `critic_flags.yaml`；只有 regex 层产生 flag 时 LLM critic 才会运行。

Legacy 路径也会把最近 8 个已撰 section 的首句滚动摘要塞进 `{prior_findings}` 槽位，并对草稿做中文字符占比检查（若 LLM 默认输出英文，则用一次硬性 `OUTPUT MUST BE IN CHINESE` 增量重试）。

#### 可选实验性 composer（仅 opt-in）

两个实验性 composer 仍受环境变量门控，**不推荐生产使用**：

- `LAZY_PAPER_AGENT=1` —— `stages/s08_section_compose/agent.py` 里的 pydantic-ai 工具调用 agent，提供 `query_kg`、`retrieve`、`check_source`、`emit_section` 四个工具。偶尔会返回元注释而非正文；遇到任何异常都会回退 legacy。
- `LAZY_PAPER_TWO_STEP=1` —— `stages/s08_section_compose/two_step.py` 的 outline → expand 流水线。把 compose 拆成大纲草稿 + 逐 bullet 展开。

两者都早于 Strategy KL，未在 v1.8 语料上重做验证。
...
## 引文处理（Citation processing）

### 设计来源

引文渲染设计借鉴自 [Onyx](https://github.com/onyx-dot-app/onyx) 的 `citation_processor.py`（MIT）。原始流式实现曾在 v1.4 vendor 到 `llm/citation/stream_processor.py`，但从未在运行时接入；项目自始至终运行的是 `llm/citation/__init__.py::process_text` 这份非流式版本。完整署名见 `THIRD_PARTY_NOTICES.md`。

### 三种渲染模式

| 模式 | 行为 | 适用场景 |
|---|---|---|
| `REMOVE` | 去除所有 `[span:doc_X:Y-Z]` 标记 —— 最终输出干净正文。 | DOCX 默认。 |
| `KEEP` | 保留标记为字面文本。 | 调试检索归因。 |
| `HYPERLINK` | 在 HTML 输出中把标记转为可点击的逐 claim 锚点，文末追加 sources 段。 | HTML 默认；对读者来说最有信任度的方式。 |

模式在 `stages/s09_render/runner.py` 中按渲染器选择：DOCX 默认 `REMOVE`，HTML 默认 `HYPERLINK`。两者均可通过 `--debug-citations`（全部强制 `KEEP`）或 `LAZY_PAPER_HTML_CITATIONS=remove`（关闭 HTML 可点击引用）覆盖。

### CLI 标志

传入 `--debug-citations` 把所有渲染器切到 `KEEP` 模式，把 `[span:...]` 作为字面文本暴露出来。用于审计 LLM 引用了哪些 chunk。

**交叉引用**：`llm/citation/__init__.py::process_text`、`stages/s09_render/renderers/{docx,html}.py`
