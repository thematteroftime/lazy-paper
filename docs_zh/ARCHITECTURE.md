...
4. **Verify**：`verify_section_draft(draft, chunks_by_id, ratio_threshold)` 用四档匹配校验每条 claim 的 `cited_quote`（精确子串 → 大小写不敏感子串 → 归一化子串：剥 LaTeX 命令 + 折叠 OCR 数字空白 → 模糊最长公共子串）。若所引 chunk 未匹配上，verifier 会扫描全部 retrieved chunk；匹配成功则接受 claim 并把 `cited_chunk_ids` 纠正（chunk-ID 容错）。阈值环境变量：`LAZY_PAPER_VERIFIER_THRESHOLD`（默认 0.85）。随后做**锚点检查**：当 claim 命名了某个 author（`X et al.` / `X 等人`）或具体数值（`2.94 J/cm³`、`91.04%` …），`cited_quote` 必须本身包含至少一个锚点 —— 否则该 quote 虽在 chunk 中存在，但并不支撑那个具体事实。
