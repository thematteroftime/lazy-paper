...
你是一位研究者（或他/她的助手），手头有一篇科研 PDF，想把它转换成结构化的多格式分析文档，但不想自己写代码。本指南会带你完成环境配置、首次运行，以及如何根据输出结果反复调整。
...
一次完整运行大约耗时 5–20 分钟，取决于论文长度和 API 延迟。每个 stage 会写入一个 `done.yaml` 标记，所以中途中断后，再跑同一条命令就能从中断处继续。
...
## 推荐的高质量模式：Strategy KL

当你需要在 benchmark 风格的论文上恢复文献引用时，请开启 **Strategy KL**。它会对每条 quote 做源端校验（先按 LaTeX/OCR 形式做归一化），每个章节跑 2 份草稿，取较好的一份；如果校验后覆盖率不达标，再用加强提示重试一次。
...
- **延迟**：s08 大约慢 1.7–2 倍（多份草稿能并行时会并行跑）。
- **质量**：verifier 会拒掉缺乏依据的 claim；触发 retry-when-empty 后会再做一次强化的 LLM 调用，把 v1.7 之前会丢掉的 comparator 引用补回来。

已在 18 论文 v1.9.2 语料 + v1.10 9 论文 variant 测试上验证：**自 v1.9.0 informed-retry 起，meng2024 T1 = 9/9/9（stdev 0）**，三次独立运行结果一致。完整数据见 `docs_zh/archive/v1_9_validation_results.md` 与 `docs/archive/v1_10_variant_comparison.md`。

只想跑快速/低成本的基线？把这些变量留空即可——默认 composer 在多数论文上效果就够用，成本只有一半。
...
2. 打开对应的 `<slug>.prompt.md`，看看喂给 LLM 的依据是什么。
3. 检查 `runs/<id>/s06_context/context.yaml` —— 如果论文的研究体系或关键词字段错了，会带偏整个生成。删掉 `s06_context/done.yaml` 重新抽取 context。
...
通常是 Homebrew 装的原生库没被找到。

1. 确认库已装：`brew list | grep -E "pango|cairo|gdk"`。
...
因为每个 stage 都会写 `done.yaml`，所以同一篇论文跑完第一次后再跑几乎零成本 —— 所有 stage 都会被跳过。只有在以下情况你才会再次产生 LLM 调用：
