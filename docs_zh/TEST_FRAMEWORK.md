...
满分 17。v1.3.3 基线（整篇上下文塞入）约能拿 12-13；v1.4.x 默认拿 0；v1.8.1 KL 3 次重测为 12 / 17 / 16（均值 15、下限 12，stdev 2.6）；**自 v1.9.0 informed-retry 起为 9 / 9 / 9（stdev 0）**；v1.11.1 保持 9/9/9 零方差。每个策略都按此打分。
...
Strategy KL 自 v1.8.1 起为推荐默认；v1.9.0 informed-retry 把 T1 方差降到 0；v1.10 Variant C 加 figure_ids 硬约束并抬升 T4。当前规范数字：

### meng2024 ch01（T1，满分 17）
...
| v1.7 KL（KG-v3 + best-of-N=2） | 13 / 1 / 1（均值 5） | 单次峰值高，但方差大 |
| v1.8.1 KL（verifier + retry 修复） | 12 / 17 / 16（均值 15，下限 12） | 方差 bug 已修 |
| **v1.9.0+ KL（informed-retry）** | **9 / 9 / 9（stdev 0）** | 逐实体诊断取代笼统提醒；零方差 |
| v1.10 Variant C（figure_ids 硬约束） | 9 / 9 / 9（stdev 0） | T1 保持；T4 在 ali2025_flash 上 4→5 |
| **v1.11.1（4 个 HIGH bug 修复）** | **9 / 9 / 9（stdev 0）** | 无回归；flagship 数值 / author / OCR-prompt / lang 修复 |

注：v1.9 之后 T1 = 9（不是 17）。早先 12–17 区间是 v1.8.x KL 随机回补的副产物；informed-retry 用对 9 个高置信模式的确定性回收替代了这个机制，停止 fuzzy 误计。分数地板从 1（v1.7 KL）抬到 9（v1.9+），代价是放弃 13–17 的运气峰值。

### 其他测试项（v1.11.x 当前）

| 用例 | 得分 | 说明 |
|---|---|---|
| T2（yang2025，不杜撰储能数据） | 3/3 ✓ | v1.7+ 起稳定 |
| T3（meng2024 ch10，合成法专属性） | 4/5 ✓ | KL 写出 tape-casting + 晶粒数据 |
| T4（ali2025_flash，比较章深度） | **5/5** 🏆 | Variant C 破基线 4 → 5 |
| T5（fu2020，通用基础） | 3/4 ✓ | |
| T6（chai2026，通用基础） | 4/4 ✓ | |

历史 v1.6/v1.7 逐策略记分牌见 `docs/archive/v1_7_validation_results.md` 与 `docs/archive/v1_8_validation_results.md`。
...
## 审计陷阱：OCR 间隔数字 + LaTeX 包裹

Cycle 12 (v1.11.1 → v1.11.2) 抓到一类常见 false-positive bug 报告：直接对
`runs/<paper>/s02_clean/doc_*.md` 跑 `grep`。MinerU OCR 经常把数值输出成
LaTeX 风格的字符流 —— `$\sim 1 7 . 3 ~ \mathrm{J/cm}^3$` 而不是 `~17.3 J/cm³`。
plain `grep "17.3"` 把这种情况全部漏抓。

Cycle-12 audit 对 `ali2025_flash` ch13 用 plain `grep`"证实" `17.3 / 20.1 / 27.9`
是 fabricated。实际上不是 —— OCR 只是把它们用空格分隔了。2 个 specialist + 2 个
meta 审计**都复现了同样的 methodology 错误**才发现。基于这个假 bug 的 v1.11.2
fix 候选删除了正确内容、引入新幻觉，最终被回退。

**审计模型输出 vs OCR 源时，必须用 `scripts/audit_grep.py` 而不是 plain `grep`。**
它把 pattern 和每行 source 都过一遍 `stages._common.normalize.normalize_ocr_latex`
（与 s08 verifier 用的归一化器同一份），LaTeX 间隔 + 单位上标变体都正确折叠。
...
audit subagent 的 prompt 也必须强制要求："比对数值 claim 与 OCR 源时，用 audit_grep.py，
不要用 plain grep"。否则同样的 false-positive pattern 会再出现一次。
...
这套纪律产出了 v1.4.2（4 路对比挑出 Strategy C 作默认）、v1.6.0（方差分析后把 Strategy J 作 opt-in）、v1.8.1（修了 KL 的稳定性 bug 后把 KL 升为推荐默认）、v1.9.0（informed-retry → stdev 0）、v1.10（3-variant × 9-paper × 3-cycle 审计选出 Variant C）以及 v1.11.1（cycle-11 sentence-level audit 出的 4 个 HIGH 修复）。
