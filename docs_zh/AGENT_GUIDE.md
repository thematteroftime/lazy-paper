...
本文档面向被指派维护、扩展或调试本仓库的 AI 编码 agent（Claude Code、Cursor、Copilot 等）。前置阅读：`docs/INTERNAL/HANDOFF.md` 和 `docs/ARCHITECTURE.md`。

## 给 agent 的 TL;DR

1. **不要动 `runs/`**，除非用户明确要求清理。它是已验证测试语料库。
2. **任何改动前后都要跑 `uv run pytest -q`**。如果你弄坏了测试，就修复或回滚。应有 250 个用例通过；live-LLM 测试通过 `-m live` 门控。
...
- **只读代码审计**（找硬编码、重复代码、未使用 import）。`subagent_type: Explore` 是理想选择 —— 把冗长审计输出挡在主上下文之外。
- **目标清晰、验收标准明确的小范围重构**（如"抽出 MIME 辅助函数，更新 2 处调用点，跑测试"）。给精确的 prompt 和具体的测试命令。

不适合：
- **主 agent 已经理解的快速编辑**。为了改 3 行而派发，纯浪费启动 token。
- **需要快速用户反馈的任务**。subagent 运行期间不可中断；用户一旦改主意，subagent 的工作就白做了。
...
- **通过在解析逻辑里硬编码特例来"修"docx 模板。** 如果模板解析错了，去修解析器（`stages/s05_template/runner.py`），而不是改模板。模板是用户数据；解析器才是你的代码。
- **为了修一篇论文而加针对该论文的硬编码逻辑。** `he2023` 失败了就修通用代码路径让它不可能再失败。如果实在必须特例化（比如已知的坏 PDF），用 flag 门控。
- **删掉你改完之后就挂的测试。** 先诊断：是你的改动错了，还是测试已过时？只有当断言不再反映原意时才更新测试。
- **不重渲已验证论文就升 prompt 版本。** 一次版本提升会让所有缓存失效 → 下一次用户运行就会全量重跑 LLM → 产生预期外的成本。
- **构建通过就把任务标记为完成。** 对于渲染器变更，"完成"包括对实际产物的视觉评审。把 PPT 转成 PNG 看一眼。
...
- **简洁**。用户已经在 PPT 布局上反复迭代了几个小时；他们不想看叙述。
- **直接陈述结果和决策**。"通过 Y 修了 X。5 篇论文已验证。已推送。"比"我已完成工作。让我解释我做了什么。"要好。
- **低风险可逆操作不要请示**（如跑测试、读文件）。但推送前、执行破坏性 `rm` 前、`git push --force` 前必须征求同意。
...
`paper_kg.parquet` 由 s06 写出一次，并且**按设计在 s06 的 `--force` 下也会保留**。KG 抽取是一次完整论文的 LLM 调用；重跑每篇论文约消耗 1 次 API 调用、耗时 15-30 秒。要强制重抽，必须显式删除该文件：
...
1. `uv run pytest -q` → 250 个通过，2 个 deselected。
