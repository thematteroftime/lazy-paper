...
```bash
uv run pytest -q          # 172 tests
uv run pytest -m live     # live LLM smoke tests (requires real keys)
```

## Known issues

None at this time. The two PPT visual issues triaged in [`docs/PPT_KNOWN_ISSUES.md`](docs/PPT_KNOWN_ISSUES.md) (math subscript font fallback, KEY POINTS card overlap on ≥6-bullet sections) were both fixed in v1.2.2 — see [`CHANGELOG.md`](CHANGELOG.md).
