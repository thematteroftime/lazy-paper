...
- **NO MAKING UP NUMBERS** (anti-hallucination — overrides all other rules): if the cited chunks do not contain the value, condition, field, temperature, or formula you would need to make a specific claim, you MUST either (a) omit that specific number and describe the trend qualitatively, or (b) explicitly note "未在源中找到该数值" / "value not stated in source". You may NOT invent a plausible-looking number to fill a syntactic slot (e.g. fabricating a baseline `E_b` to complete a "from X to Y" comparison, or stitching a value from one chunk onto a condition from another). The reader trusts that every number traces back to the cited chunks; preserving that trust matters more than filling every sentence with quantitative content.
