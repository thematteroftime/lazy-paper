...
def _promotable(title: str, idx: int) -> tuple[str, str | None]:
    """Make `f"{idx} {title}"` survive s05's guidance heuristics.

    Repairs, in order: arrow notation stripped; too-short heading lines padded;
    action-verb / otherwise-demotable titles fall back to a generic heading
    with the original title preserved as a guidance line (returned second).
    """
    from stages.s05_template.runner import _is_guidance_line

    is_cjk = bool(re.search(r"[一-鿿]", title))
    t = title.replace("->", "-").replace("→", "-").strip()
    if not _is_guidance_line(f"{idx} {t}"):
        return t, None
    padded = t + ("·总览" if is_cjk else " Overview")
    if not _is_guidance_line(f"{idx} {padded}"):
        return padded, None
    return (f"第{idx}部分" if is_cjk else f"Part {idx}"), title
...
def write_docx(sections: list[dict], out_path: Path, *, idea: str) -> None:
    ...
    promotes — and pre-checked against s05's own demotion heuristics via
    `_promotable`. Questions are plain paragraphs -> guidance. The preamble
    line starts with "(" so s05 ignores it (guidance prefix, no open heading).
    """
    ...
    for i, sec in enumerate(sections, 1):
        title, demoted_original = _promotable(sec["title"], i)
        doc.add_paragraph(f"{i} {title}")
        if demoted_original:
            doc.add_paragraph(f"- {demoted_original}")
...
def roundtrip_check(out_path: Path, sections: list[dict]) -> list[dict]:
    ...
    if len(nodes) != len(sections):
        raise SystemExit(
            ...
            f"s05 parsed {len(nodes)}. LLM output varies — rerun the command "
            f"or edit the docx titles; if it reproduces, please report it")
    return nodes
